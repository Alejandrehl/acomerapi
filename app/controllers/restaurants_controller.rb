class RestaurantsController < ApplicationController
  before_action :set_restaurant, only: [:show, :edit, :update, :destroy]
  load_and_authorize_resource :except => [:show]
  include ApplicationHelper

  # GET /restaurants
  # GET /restaurants.json
  def index
    @restaurants = current_user.restaurants
  end

  # GET /restaurants/1
  # GET /restaurants/1.json
  def show
    @restaurant = Restaurant.find(params[:id])
    @branches = @restaurant.branches
  end

  # GET /restaurants/new
  def new
    @restaurant = Restaurant.new
  end

  # GET /restaurants/1/edit
  def edit
  end

  # POST /restaurants
  # POST /restaurants.json
  def create
    @restaurant = Restaurant.new(restaurant_params)

      image = params[:restaurant][:logo_url].tempfile
      image_name = params[:restaurant][:logo_url].original_filename
      image_type = params[:restaurant][:logo_url].content_type
    
      # Se asigna la respuesta del helper a una variable.
      @url = uploadImageToS3(image, image_name, image_type)
    
      # Se pregunta si la variable trae datos
      if !@url.nil?
        @restaurant.image = @url
      end
    

    respond_to do |format|
      if @restaurant.save
        format.html { redirect_to @restaurant, notice: 'Restaurant was successfully created.' }
        format.json { render :show, status: :created, location: @restaurant }
      else
        format.html { render :new }
        format.json { render json: @restaurant.errors, status: :unprocessable_entity }
      end
    end
  end

  # PATCH/PUT /restaurants/1
  # PATCH/PUT /restaurants/1.json
  def update
    # Cambio de estado desde panel de usuario con rol de operations
    if params[:status]
      respond_to do |format|
        @restaurant.status = params[:status]
        @restaurant.save
        return redirect_to operations_restaurants_path
      end
    end

    respond_to do |format|
      if @restaurant.update(restaurant_params)
        format.html { redirect_to @restaurant, notice: 'Restaurant was successfully updated.' }
        format.json { render :show, status: :ok, location: @restaurant }
      else
        format.html { render :edit }
        format.json { render json: @restaurant.errors, status: :unprocessable_entity }
      end
    end
  end

  # DELETE /restaurants/1
  # DELETE /restaurants/1.json
  def destroy
    @restaurant.branches.each do |branch|
      branch.destroy
    end

    @restaurant.destroy
    respond_to do |format|
      format.html { redirect_to restaurants_url, notice: 'Restaurant eliminado correctamente.' }
      format.json { head :no_content }
    end
  end

  private
    # Use callbacks to share common setup or constraints between actions.
    def set_restaurant
      @restaurant = Restaurant.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def restaurant_params
      params.require(:restaurant).permit(:name, :description, :business_name, :giro, :telephone, :email, :instagram, :facebook, :twitter, :pinterest, :tripadvisor, :image ,:user_id)
    end
end
